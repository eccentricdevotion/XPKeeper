...
public class WithdrawCommandNode {

    private final XPKeeper plugin;

    public WithdrawCommandNode(XPKeeper plugin) {
        this.plugin = plugin;
    }

    public LiteralCommandNode<CommandSourceStack> build() {
        LiteralArgumentBuilder<CommandSourceStack> command = Commands.literal("xpkwithdraw")
                ...
                .then(Commands.argument("amount", IntegerArgumentType.integer())
                        .executes(ctx -> {
                            ...
                            plugin.saveConfig();
                            XPKUtils.xpkMessage(ctx.getSource().getSender(), " withdraw config value set to: " + amount);
